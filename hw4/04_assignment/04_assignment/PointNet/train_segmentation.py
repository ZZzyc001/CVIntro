...
if __name__ == '__main__':
    ...
    dataset = ShapeNetSegmentationDataset(root=opt.dataset,
    ...
    test_dataset = ShapeNetSegmentationDataset(root=opt.dataset,
                                               npoints=opt.num_points,
                                               class_choice=["Airplane"],
                                               split='test',
    ...
    classifier = PointNetSeg(k=num_classes).cuda()
    ...
    for i, data in tqdm(enumerate(testdataloader, 0)):
        ...
        for shape_idx in range(target_np.shape[0]):
            ...
            for part in parts:
                I = np.sum(
                    np.logical_and(pred_np[shape_idx] == part,
                                   target_np[shape_idx] == part))
                U = np.sum(
                    np.logical_or(pred_np[shape_idx] == part,
                                  target_np[shape_idx] == part))
                if U == 0:
