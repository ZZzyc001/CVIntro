...
def bn_forward_train(x, gamma, beta):
    ...
    global mean, var, momentum

    mean = momentum * mean + (1 - momentum) * sample_mean
    var = momentum * var + (1 - momentum) * sample_var
    # ----------------TODO------------------
...
def bn_backward(dout, cache):
    ...
    center = x - sample_mean
    dx_hat = dout * gamma
    s = np.sqrt(sample_var + eps)
    dcenter = dx_hat / s + np.sum(dx_hat * center,
                                  axis=0) * (-1 / s**3) * center / N
    dx = dcenter - np.mean(dcenter, axis=0)
    # ----------------TODO------------------
...
if __name__ == "__main__":
    ...
    for i in range(9):
        train_data[i, :] = cv2.imread("mnist_subset/" + str(i) + ".png",
                                      cv2.IMREAD_GRAYSCALE).reshape(-1) / 255.
    gt_y = np.zeros((9, 1))
    gt_y[0] = 1

    val_data = np.zeros((1, 784))
    val_data[0, :] = cv2.imread("mnist_subset/9.png",
                                cv2.IMREAD_GRAYSCALE).reshape(-1) / 255.
    val_gt = np.zeros((1, 1))
    ...
    for i in range(50):
        ...
        # Backward : compute the gradient of paratmerters of layer1
        # (grad_layer_1) and layer2 (grad_layer_2)
